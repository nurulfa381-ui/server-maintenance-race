...
const G = {
    ...
    showQuestion() {
        ...
        if (!question) {
            ...
            this.saveStudentProfile({ finished: true });
    ...
    saveStudentProfile(extra = {}) {
        ...
        const totalQuestions =
            Array.isArray(window.SERVER_HERO_QUESTIONS)
                ? window.SERVER_HERO_QUESTIONS.length
                : 10;
        ...
        const average = answered
            ? Math.round((this.correctCount / answered) * 100)
            : 0;

        const profile = {
            name: this.player.name || "NAMA PELAJAR",
            id: this.player.id || "ID001",
            avatar: this.player.avatar || this.avatar || "🧑‍💻",
            room: this.room || "",
            score: this.score || 0,
            xp: this.xp || 0,
            coins: Math.floor((this.score || 0) / 100),
            completedMission: answered,
            totalMission: totalQuestions,
            average,
            correctAnswers: this.correctCount || 0,
            wrongAnswers: this.wrongCount || 0,
            fastAnswers: this.fastCount || 0,
        ...
        try {
            ...
            console.warn("Profil pelajar tidak dapat disimpan.", error);
        }
    },
    ...
    finish(playersObject) {
        ...
        this.saveStudentProfile({
            bestRank: position > 0 ? position : 0,
            finished: true
        });
